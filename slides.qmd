...
-   Hybrid method - combines multiple feature selection methods 
-   Combines filter and wrapper methods

Step 1: filtering using the correlation coefficient. (e.g. trim the
"low hanging fruit") 

Step 2: remove variables iteratively using RFE.
...
-   Five Sentiment label classes, ranging from extremely positive to
    extremely negative
-   Recoded to a binary classification task, positive (1) or negative
    (0)
...
-   difference between accuracy on the training and test set
    (overtraining)
...
CFS + FRE: Remove 20% of variables with lowest correlation, Select best of 20 sized subsets using cross
validation
